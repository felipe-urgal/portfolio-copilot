...
describe("AssetQuantity", () => {
  it("represents zero and positive quantities exactly at 12 decimal places", () => {
    ...
    expect(AssetQuantity.fromDecimal("0.00000001").toDecimalString()).toBe("0.000000010000");
    expect(AssetQuantity.fromDecimal("1.234567890123").toDecimalString()).toBe("1.234567890123");
  });

  it("normalizes harmless signs and leading zeros without binary floating point", () => {
    expect(AssetQuantity.fromDecimal("+00012.3400").toDecimalString()).toBe("12.340000000000");
    expect(AssetQuantity.fromDecimal("-0.000000000000").toDecimalString()).toBe("0.000000000000");
  });

  it("rejects negative, malformed or over-precision quantities instead of rounding", () => {
    for (const value of ["-0.000000000001", "1.2345678901234", "1e-8", "1,5", "", "abc"]) {
