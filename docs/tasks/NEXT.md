...
- o retry de browser QA foi diagnosticado com maior precisão: o shell desta sessão não possui `agent-browser` instalado e não resolve o hostname público antigo, portanto screenshots, fidelity ledger, keyboard-only E2E e screen-reader smoke continuam **não executados**;
- a inspeção do projeto Vercel confirmou que deployments Git-triggered recentes aparecem como `CANCELED` porque `vercel.json` define deliberadamente `git.deploymentEnabled=false`; esse contrato foi introduzido no #145 para evitar deploy duplicado no Hobby e delegar a promoção ao Dev Dashboard, portanto não é regressão de configuração;
- o próximo débito operacional real é reconciliar `docs/PRODUCTION.md` com o fluxo atual `check -> migrate -> provider-deploy -> verify`, substituindo a descrição antiga que ainda sugere deploy automático após merge;
- outro finding real já reservado para a próxima vertical é o seletor local de tarefas da Carteira: ele troca painel dentro da mesma rota, mas usa `aria-current="page"`; a correção deve migrar para semântica de seleção apropriada, preservar `aria-controls`, atualizar regressão e reconciliar `docs/design/PORTFOLIO.md`.
